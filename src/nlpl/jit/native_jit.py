...
Compiles individual NLPL functions to native machine code using the existing
...
class NativeFunctionCache:
    ...
    def __init__(self) -> None:
        ...
        self._tmpdir = tempfile.mkdtemp(prefix="nlpl_native_jit_")
...
class NativeFunctionJIT:
    """
    Compiles a single NLPL function to native machine code and caches it.
    ...
    def __init__(self, interpreter: Any, opt_level: int = 3) -> None:
        """
        Args:
            interpreter: The NLPL interpreter instance.  Used to resolve
    ...
    def _compile(
        ...
        # The exported symbol name is "nlpl_<func_name>" when we generate IR
        # for JIT use (see _generate_ir below).
        symbol_name = f"nlpl_{func_name}"
        try:
    ...
    def _generate_ir(self, func_name: str, func_def: Any) -> Optional[str]:
        ...
        # Collect the function and all recursively-called NLPL functions
    ...
    def _collect_callees(
        ...
        """
        Return a dict of {name: func_def} for root_def and all NLPL
