...
NLPL Test Runner
================
Discovers, filters, and executes NLPL test programs.  Supports parallel
...
Usage (CLI via this module):
    python -m nlpl.tooling.test_runner test_programs/ --filter "my_suite*"
    python -m nlpl.tooling.test_runner test_programs/ --format tap
    python -m nlpl.tooling.test_runner test_programs/ --format json --workers 8
"""
...
def _run_file_collect(path: str, name_filter: Optional[str],
    ...
    try:
        ...
        from nlpl.compiler.preprocessor import preprocess_ast, host_target
...
class TestRunner:
    """
    Discover and execute NLPL test programs.

    Parameters
    ----------
    workers:
        Number of parallel workers.  ``1`` means sequential execution.
        Workers share the GIL but NLPL test files are I/O-bound during
    ...
    pattern:
        Glob pattern for file discovery, e.g. ``"test_*.nlpl"`` (default).
    name_filter:
    ...
    def run_files(self, paths: List[str]) -> Dict:
        """Execute a list of NLPL test files and return a RunSummary."""
        start = time.time()
...
def _cli():
    ...
    ap.add_argument("--pattern", default="test_*.nlpl",
                    help="Glob pattern for test file discovery (default: test_*.nlpl)")
