/*
 * nlpl_collections.c -- NLPL growable array / collection runtime.
 *
...
static void _array_grow(NLPLArray *arr, int64_t min_capacity)
{
    ...
    if (!new_data) {
        nlpl_panic("NLPLArray: memory allocation failed during growth");
    }
    arr->data     = new_data;
...
NLPLArray *nlpl_array_new(void)
{
    ...
    if (!arr) {
        nlpl_panic("nlpl_array_new: memory allocation failed");
    }
    arr->data     = NULL;
    arr->length   = 0;
    arr->capacity = 0;
...
{
    if (!arr) {
        nlpl_panic("nlpl_array_push: null array");
    }
...
{
    if (!arr || arr->length == 0) {
        nlpl_panic("pop from empty array");
    }
...
{
    if (!arr) {
        nlpl_panic("nlpl_array_get: null array");
    }
    if (idx < 0 || idx >= arr->length) {
        nlpl_panic("array index out of range");
    }
    return arr->data[idx];
}
...
{
    if (!arr) {
        nlpl_panic("nlpl_array_set: null array");
    }
    if (idx < 0 || idx >= arr->length) {
        nlpl_panic("array index out of range");
    }
...
{
    if (!arr) {
        nlpl_panic("nlpl_array_slice: null array");
    }
    if (start < 0 || end < start || end > arr->length) {
        nlpl_panic("array slice index out of range");
    }
    int64_t new_len = end - start;
    NLPLArray *result = nlpl_array_new();
    if (new_len > 0) {
...
{
    if (!arr) {
        nlpl_panic("nlpl_array_copy: null array");
    }
    NLPLArray *result = nlpl_array_new();
    if (arr->length > 0) {
...
int64_t *arrpush(int64_t *arr, int64_t count, int64_t elem)
{
    ...
    if (!new_arr) {
        nlpl_panic("arrpush: memory allocation failed");
    }
    if (arr && count > 0) {
...
{
    if (!arr || count <= 0) {
        nlpl_panic("arrpop: pop from empty array");
    }
    int64_t new_count = count - 1;
    if (new_count == 0) {
        ...
        if (!new_arr) {
            nlpl_panic("arrpop: memory allocation failed");
        }
        return new_arr;
    }
    int64_t *new_arr = (int64_t *)malloc((size_t)new_count * sizeof(int64_t));
...
{
    if (!arr) {
        nlpl_panic("arrslice: null array");
    }
    if (start < 0 || end < start) {
        nlpl_panic("arrslice: invalid range");
    }
    int64_t len = end - start;
    if (len == 0) {
        int64_t *new_arr = (int64_t *)malloc(sizeof(int64_t));
        if (!new_arr) {
            nlpl_panic("arrslice: memory allocation failed");
        }
        return new_arr;
    }
