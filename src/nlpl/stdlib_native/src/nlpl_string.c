/*
 * nlpl_string.c -- NLPL string operations.
 *
...
{
    if (!str) {
        nlpl_panic("nlpl_substr: null string");
    }
    int64_t src_len = (int64_t)strlen(str);
    if (start < 0 || length < 0 || start + length > src_len) {
        nlpl_panic("substring index out of range");
    }
    char *result = (char *)malloc((size_t)(length + 1));
    if (!result) {
        nlpl_panic("nlpl_substr: memory allocation failed");
    }
...
{
    if (!str) {
        nlpl_panic("nlpl_charat: null string");
    }
    int64_t len = (int64_t)strlen(str);
    if (index < 0 || index >= len) {
        nlpl_panic("character index out of range");
    }
    char *result = (char *)malloc(2);
    if (!result) {
        nlpl_panic("nlpl_charat: memory allocation failed");
    }
    result[0] = str[index];
    result[1] = '\0';
    return result;
...
{
    if (!str) {
        nlpl_panic("nlpl_str_replace: null string");
    }
    if (!old_sub || !new_sub) {
        return strdup(str);
    }
    ...
    if (old_len == 0) {
        ...
        if (!result) {
            nlpl_panic("nlpl_str_replace: memory allocation failed");
        }
        return result;
    }
...
{
    if (!str) {
        nlpl_panic("nlpl_str_trim: null string");
    }
    ...
    if (!result) {
        nlpl_panic("nlpl_str_trim: memory allocation failed");
    }
...
/* ===========================================================================
 * nlpl_str_toupper / nlpl_str_tolower
 * =========================================================================*/

char *nlpl_str_toupper(const char *str)
{
    if (!str) {
        nlpl_panic("nlpl_str_toupper: null string");
    }
    size_t len = strlen(str);
    char *result = (char *)malloc(len + 1);
    if (!result) {
        nlpl_panic("nlpl_str_toupper: memory allocation failed");
    }
...
{
    if (!str) {
        nlpl_panic("nlpl_str_tolower: null string");
    }
    size_t len = strlen(str);
    char *result = (char *)malloc(len + 1);
    if (!result) {
        nlpl_panic("nlpl_str_tolower: memory allocation failed");
    }
    for (size_t i = 0; i < len; i++) {
...
char *nlpl_str_concat(const char *a, const char *b)
{
    ...
    if (!result) {
        nlpl_panic("nlpl_str_concat: memory allocation failed");
    }
    memcpy(result, a, la);
...
char *nlpl_str_repeat(const char *str, int64_t times)
{
    if (!str || times <= 0) {
        ...
        if (!result) {
            nlpl_panic("nlpl_str_repeat: memory allocation failed");
        }
        result[0] = '\0';
        return result;
    }
...
/* ===========================================================================
 * nlpl_str_starts_with / nlpl_str_ends_with
 * =========================================================================*/
...
char *nlpl_str_format_int(const char *fmt, int64_t n)
{
    ...
    if (!result) {
        nlpl_panic("nlpl_str_format_int: memory allocation failed");
    }
    return result;
}
