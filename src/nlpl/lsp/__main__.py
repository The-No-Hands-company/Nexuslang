"""
NLPL Language Server - Main Entry Point
========================================

Run the NLPL Language Server Protocol server.

Usage:
    python -m nlpl.lsp
    python -m nlpl.lsp --stdio (default)
    python -m nlpl.lsp --tcp --port 5007
"""

import sys
import argparse
import logging
from .server import NLPLLanguageServer


def main():
    """Main entry point for the NLPL Language Server."""
    parser = argparse.ArgumentParser(
        description='NLPL Language Server Protocol Implementation',
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  python -m nlpl.lsp                    # Start LSP server on stdio
  python -m nlpl.lsp --tcp --port 5007  # Start LSP server on TCP port
  python -m nlpl.lsp --debug            # Enable debug logging
        """
    )
    
    parser.add_argument(
        '--stdio',
        action='store_true',
        default=True,
        help='Use stdio for communication (default)'
    )
    
    parser.add_argument(
        '--tcp',
        action='store_true',
        help='Use TCP socket for communication'
    )
    
    parser.add_argument(
        '--port',
        type=int,
        default=5007,
        help='TCP port to listen on (default: 5007)'
    )
    
    parser.add_argument(
        '--host',
        type=str,
        default='127.0.0.1',
        help='TCP host to bind to (default: 127.0.0.1)'
    )
    
    parser.add_argument(
        '--debug',
        action='store_true',
        help='Enable debug logging to /tmp/nlpl-lsp.log'
    )
    
    parser.add_argument(
        '--log-file',
        type=str,
        default='/tmp/nlpl-lsp.log',
        help='Log file path (default: /tmp/nlpl-lsp.log)'
    )
    
    args = parser.parse_args()
    
    # Configure logging
    if args.debug:
        logging.basicConfig(
            filename=args.log_file,
            level=logging.DEBUG,
            format='%(asctime)s - %(name)s - %(levelname)s - %(message)s'
        )
        logger = logging.getLogger('nlpl-lsp')
        logger.info(f"Starting NLPL Language Server (debug mode)")
        logger.info(f"Args: {args}")
    
    # Create and start server
    server = NLPLLanguageServer()
    
    if args.tcp:
        # TCP mode - not yet implemented
        print(f"TCP mode not yet implemented. Use --stdio (default).", file=sys.stderr)
        sys.exit(1)
    else:
        # STDIO mode (default)
        try:
            server.start()
        except KeyboardInterrupt:
            if args.debug:
                logger = logging.getLogger('nlpl-lsp')
                logger.info("Server interrupted by user")
            sys.exit(0)
        except Exception as e:
            if args.debug:
                logger = logging.getLogger('nlpl-lsp')
                logger.error(f"Server error: {e}", exc_info=True)
            print(f"Error: {e}", file=sys.stderr)
            sys.exit(1)


if __name__ == '__main__':
    main()
