...
def initialize_nlpl_modules():
    """
    Initialize NLPL modules using direct loading to avoid Python 3.14 import bugs.
    
    This function manually loads the core NLPL modules in the correct order,
    bypassing the standard import system.
    """
    # Get the src/nlpl directory
    nlpl_dir = Path(__file__).parent
    
    # Load modules in dependency order
    modules_to_load = [
        ('nlpl.parser.lexer', nlpl_dir / 'parser' / 'lexer.py'),
        ('nlpl.parser.ast', nlpl_dir / 'parser' / 'ast.py'),
        ('nlpl.parser.parser', nlpl_dir / 'parser' / 'parser.py'),
    ]
