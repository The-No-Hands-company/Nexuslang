"""
C Header Parser for NLPL FFI.
Parses C header files to automatically generate NLPL extern declarations.
...
class CParameter:
    ...
    def to_nlpl_param(self) -> str:
        """Generate NLPL parameter declaration."""
        if self.name:
            return f"{self.name} as {self.nlpl_type}"
        else:
            return f"as {self.nlpl_type}"
...
class CFunctionDeclaration:
    ...
    def to_nlpl_declaration(self) -> str:
        """Generate NLPL extern function declaration."""
        parts = [f"extern function {self.name}"]
        
        if self.parameters:
            param_strs = []
            for param in self.parameters:
                param_strs.append(param.to_nlpl_param())
            parts.append(" with " + " and ".join(param_strs))
        
        parts.append(f" returns {self.nlpl_return_type}")
...
class CEnumDeclaration:
    ...
    def to_nlpl_constants(self) -> List[str]:
        """Generate NLPL constant declarations for enum values."""
        constants = []
...
class TypeMapper:
    """Maps C types to NLPL types and vice versa."""
    
    def __init__(self):
        # C fundamental types -> NLPL types
        self.c_to_nlpl = {
            # Integer types
            'char': 'Integer',
            'signed char': 'Integer',
    ...
    def add_custom_mapping(self, c_type: str, nlpl_type: str):
        """Add a custom C->NLPL type mapping."""
        self.custom_mappings[c_type] = nlpl_type
    
    def c_to_nlpl_type(self, c_type: str) -> str:
        """Convert C type to NLPL type."""
        # Strip whitespace
        c_type = c_type.strip()
        ...
        if base_type in self.c_to_nlpl:
            return self.c_to_nlpl[base_type]
        
        # Default: treat as opaque pointer
        return 'Pointer'
    
    def nlpl_to_c_type(self, nlpl_type: str) -> str:
        """Convert NLPL type to C type."""
        if nlpl_type in self.nlpl_to_c:
            return self.nlpl_to_c[nlpl_type]
        
        # Handle struct/union types
        if nlpl_type.startswith("Struct_"):
            struct_name = nlpl_type[7:]
            return f"struct {struct_name}"
        
        if nlpl_type.startswith("Union_"):
            union_name = nlpl_type[6:]
...
class CHeaderParser:
    ...
    Generates NLPL extern declarations automatically.
    """
    ...
    def _extract_functions(self, content: str, header_file: str):
        ...
        for match in re.finditer(pattern, content, re.VERBOSE | re.MULTILINE):
            ...
            # Convert return type
            nlpl_return = self.type_mapper.c_to_nlpl_type(return_type)
            if nlpl_return == 'Void':
                nlpl_return = 'nothing'
    ...
    def _parse_parameters(self, params_str: str) -> List[CParameter]:
        ...
        for param in param_list:
            ...
            if tokens:
                ...
                nlpl_type = self.type_mapper.c_to_nlpl_type(c_type)
    ...
    def _extract_typedefs(self, content: str, header_file: str):
        ...
        for match in re.finditer(pattern, content):
            ...
            nlpl_type = self.type_mapper.c_to_nlpl_type(original_type)
            ...
            self.type_mapper.add_custom_mapping(alias_name, nlpl_type)
    ...
    def generate_nlpl_module(self, output_path: Optional[str] = None) -> str:
        """
        Generate complete NLPL module with all extern declarations.
        
        Args:
            output_path: Optional file path to write output
        
        Returns:
            Generated NLPL module source code
        """
        lines = []
        
        # Header comment
        lines.append("# Auto-generated NLPL FFI bindings")
        if self.library_name:
            lines.append(f"# Library: {self.library_name}")
        lines.append("# Generated by NLPL C Header Parser")
        lines.append("")
        ...
        if self.enums:
            ...
            for enum in self.enums:
                ...
                lines.extend(enum.to_nlpl_constants())
                lines.append("")
        ...
        if self.structs:
            ...
            for struct in self.structs:
                lines.append(struct.to_nlpl_declaration())
                lines.append("")
        ...
        if self.functions:
            ...
            for func in self.functions:
                lines.append(func.to_nlpl_declaration())
                lines.append("")
        ...
        if output_path:
            with open(output_path, 'w', encoding='utf-8') as f:
                f.write(output)
            print(f"Generated NLPL bindings: {output_path}")
...
def parse_c_header(header_path: str, library_name: Optional[str] = None, 
    ...
    """
    Convenience function to parse a C header and generate NLPL bindings.
    ...
    if parser.parse_header(header_path):
        parser.generate_nlpl_module(output_path)
