"""
ABI Compatibility Checker for NLPL FFI.

Validates that NLPL type declarations match actual C ABI expectations,
...
# NLPL type name -> canonical C type name
...
class CTypesVerifier:
    ...
    Builds a ctypes Structure subclass matching the NLPL declaration and
    ...
    def nlpl_to_ctypes(self, type_name: str) -> Optional[Any]:
        """Convert an NLPL or C type name to a ctypes type."""
        ...
        for layout in computed_layouts:
            ...
            if runtime_offset != layout.offset:
                issues.append(ABIIssue(
                    ...
                    message="Field offset mismatch between NLPL declaration and platform ABI",
...
class FunctionSignatureDecl:
    """An NLPL extern function declaration to be ABI-checked."""
    name: str
...
class ABICompatibilityChecker:
    ...
    def check_enum(
        ...
        Args:
            ...
            declared_underlying_type: NLPL type name for the backing integer.
