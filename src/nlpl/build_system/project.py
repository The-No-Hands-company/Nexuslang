...
@dataclass
class ProjectConfig:
    """Project configuration from nlpl.toml."""
    name: str
    version: str
    ...
    def from_toml(cls, toml_path: Path) -> 'ProjectConfig':
        """Load project config from nlpl.toml."""
        ...
        for name, spec in targets.items():
            target = Target(
                name=name,
                source=spec.get('source', f'src/{name}.nlpl'),
...
class Project:
    """Represents an NLPL project."""
    
    def __init__(self, root_path: Path):
        self.root_path = root_path
        self.config_path = root_path / "nlpl.toml"
    ...
    def init(cls, root_path: Path, name: str, **kwargs) -> 'Project':
        ...
        # Save config
        config.to_toml(root_path / "nlpl.toml")
        
        # Create main.nlpl
        main_path = root_path / "src" / "main.nlpl"
        with open(main_path, 'w') as f:
            f.write(f'''# {name}
# Created by NLPL Build System
