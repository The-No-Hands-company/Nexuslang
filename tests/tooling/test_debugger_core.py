...
Tests for the NLPL core debugger: breakpoint management, step execution,
...
class TestBreakpointManagement:
    def test_add_breakpoint_returns_object(self):
        ...
        bp = dbg.add_breakpoint("test.nlpl", 10)
        assert isinstance(bp, Breakpoint)
        assert bp.file == os.path.normpath("test.nlpl")
        assert bp.line == 10
    ...
    def test_add_breakpoint_with_condition(self):
        dbg, _ = _make_debugger()
        bp = dbg.add_breakpoint("file.nlpl", 5, condition="x > 0")
        assert bp.condition == "x > 0"

    def test_add_temporary_breakpoint(self):
        dbg, _ = _make_debugger()
        bp = dbg.add_breakpoint("file.nlpl", 3, temp=True)
        assert bp.temp is True

    def test_duplicate_breakpoint_overwrites(self):
        dbg, _ = _make_debugger()
        dbg.add_breakpoint("file.nlpl", 10)
        dbg.add_breakpoint("file.nlpl", 10, condition="y == 1")
    ...
    def test_list_breakpoints_sorted(self):
        ...
        a_lines = [bp.line for bp in bps if "a.nlpl" in bp.file]
        assert a_lines == sorted(a_lines)

    def test_path_normalisation(self):
        dbg, _ = _make_debugger()
        dbg.add_breakpoint("./dir/../file.nlpl", 1)
        assert len(dbg.list_breakpoints()) == 1
    ...
    def test_temporary_breakpoint_removed_after_check(self):
        dbg, _ = _make_debugger()
        dbg.add_breakpoint("file.nlpl", 5, temp=True)
        bp = dbg._check_breakpoint("file.nlpl", 5)
        assert bp is not None
        assert len(dbg.list_breakpoints()) == 0  # Auto-removed

    def test_disabled_breakpoint_not_triggered(self):
        dbg, _ = _make_debugger()
        dbg.add_breakpoint("file.nlpl", 5)
        dbg.toggle_breakpoint("file.nlpl", 5)  # Disable
        bp = dbg._check_breakpoint("file.nlpl", 5)
        assert bp is None
...
class TestShouldPause:
    def test_pause_at_breakpoint(self):
        dbg, _ = _make_debugger()
        dbg.add_breakpoint("file.nlpl", 10)
        assert dbg._should_pause("file.nlpl", 10) is True

    def test_no_pause_without_breakpoint_in_running_state(self):
        dbg, _ = _make_debugger()
        assert dbg._should_pause("file.nlpl", 5) is False

    def test_pause_in_stepping_state(self):
        dbg, _ = _make_debugger()
        dbg.state = DebuggerState.STEPPING
        assert dbg._should_pause("file.nlpl", 99) is True
    ...
    def test_step_over_pauses_at_same_depth(self):
        ...
        dbg.target_depth = 2
        assert dbg._should_pause("file.nlpl", 1) is True
    ...
    def test_step_over_does_not_pause_in_deeper_frame(self):
        ...
        dbg.state = DebuggerState.STEP_OVER
        dbg.target_depth = 2
        assert dbg._should_pause("file.nlpl", 1) is False
...
class TestCallStack:
    ...
    def test_push_frame(self):
        dbg, _ = _make_debugger()
        dbg.push_frame("my_func", "file.nlpl", 10, {"x": 1})
        frame = dbg.current_frame()
        assert frame is not None
        assert frame.function_name == "my_func"
        assert frame.file == "file.nlpl"
    ...
    def test_push_frame_increments_depth(self):
        ...
        dbg.push_frame("f", "f.nlpl", 1, {})
        assert dbg.step_depth == 1
        dbg.push_frame("g", "f.nlpl", 2, {})
        assert dbg.step_depth == 2

    def test_pop_frame(self):
        dbg, _ = _make_debugger()
        dbg.push_frame("f", "f.nlpl", 1, {})
        dbg.push_frame("g", "f.nlpl", 5, {})
        dbg.pop_frame()
    ...
    def test_local_vars_copied_on_push(self):
        dbg, _ = _make_debugger()
        original = {"a": 1}
        dbg.push_frame("f", "f.nlpl", 1, original)
        original["b"] = 2
        assert "b" not in dbg.current_frame().local_vars

    def test_multiple_frames(self):
        dbg, _ = _make_debugger()
        dbg.push_frame("main", "a.nlpl", 1, {})
        dbg.push_frame("helper", "a.nlpl", 10, {})
        dbg.push_frame("inner", "a.nlpl", 20, {})
...
class TestTraceHooks:
    def test_trace_line_increments_total_steps(self):
        dbg, _ = _make_debugger()
        dbg.trace_line("file.nlpl", 1)
        dbg.trace_line("file.nlpl", 2)
        assert dbg.total_steps == 2

    def test_trace_line_updates_current_location(self):
        dbg, _ = _make_debugger()
        dbg.trace_line("src.nlpl", 15)
        assert dbg.current_file == "src.nlpl"
    ...
    def test_trace_line_calls_step_callback(self):
        ...
        on_step = MagicMock()
        dbg.on_step = on_step
        dbg.trace_line("f.nlpl", 7)
        on_step.assert_called_once_with("f.nlpl", 7)

    def test_trace_call_pushes_frame(self):
        dbg, _ = _make_debugger()
        dbg.trace_call("foo", "a.nlpl", 3, {"arg": 1})
        assert dbg.current_frame().function_name == "foo"

    def test_trace_return_pops_frame(self):
        dbg, _ = _make_debugger()
        dbg.trace_call("foo", "a.nlpl", 3, {})
    ...
    def test_breakpoint_hit_count_increment(self):
        dbg, _ = _make_debugger()
        dbg.add_breakpoint("file.nlpl", 5)
        dbg._check_breakpoint("file.nlpl", 5)
        dbg._check_breakpoint("file.nlpl", 5)
        bp = dbg.list_breakpoints()[0]
    ...
    def test_breakpoints_hit_stat_tracked(self):
        dbg, _ = _make_debugger()
        dbg.add_breakpoint("file.nlpl", 2)
        dbg.add_breakpoint("file.nlpl", 4)
        dbg._check_breakpoint("file.nlpl", 2)
        dbg._check_breakpoint("file.nlpl", 4)
...
class TestStatistics:
    ...
    def test_stats_after_execution(self):
        dbg, _ = _make_debugger()
        dbg.add_breakpoint("a.nlpl", 1)
        dbg.trace_line("a.nlpl", 1)
        dbg.trace_line("a.nlpl", 2)
        assert dbg.total_steps == 2
...
class TestBreakpointDataclass:
    def test_str_representation(self):
        bp = Breakpoint(file="test.nlpl", line=10)
        s = str(bp)
    ...
    def test_str_with_condition(self):
        bp = Breakpoint(file="f.nlpl", line=1, condition="x > 10")
        assert "x > 10" in str(bp)

    def test_str_temporary(self):
        bp = Breakpoint(file="f.nlpl", line=1, temp=True)
...
class TestCallFrame:
    def test_str_representation(self):
        frame = CallFrame(function_name="greet", file="main.nlpl", line=5)
        s = str(frame)
        assert "greet" in s
        assert "main.nlpl" in s
        assert "5" in s

    def test_default_local_vars_empty(self):
        frame = CallFrame(function_name="f", file="f.nlpl", line=1)
