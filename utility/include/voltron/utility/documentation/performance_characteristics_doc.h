#pragma once

#include <string>
#include <vector>
#include <functional>
#include <source_location>

namespace voltron::utility::documentation {

/**
 * @brief Document performance complexity
 * 
 * Part of the Voltron C++ Utility Toolkit - comprehensive diagnostic infrastructure
 * for professional C++23 development.
 * 
 * @category Documentation
 * @version 1.0.0
 */
class PerformanceCharacteristicsDoc {
public:
    /**
     * @brief Get singleton instance
     */
    static PerformanceCharacteristicsDoc& instance();

    /**
     * @brief Initialize the utility
     * @param config Optional configuration parameters
     */
    void initialize(const std::string& config = "");

    /**
     * @brief Shutdown the utility and cleanup resources
     */
    void shutdown();

    /**
     * @brief Check if the utility is currently enabled
     */
    bool isEnabled() const;

    /**
     * @brief Enable the utility
     */
    void enable();

    /**
     * @brief Disable the utility
     */
    void disable();

    /**
     * @brief Get utility statistics/status
     */
    std::string getStatus() const;

    /**
     * @brief Reset utility state
     */
    void reset();

private:
    PerformanceCharacteristicsDoc() = default;
    ~PerformanceCharacteristicsDoc() = default;
    
    // Non-copyable, non-movable
    PerformanceCharacteristicsDoc(const PerformanceCharacteristicsDoc&) = delete;
    PerformanceCharacteristicsDoc& operator=(const PerformanceCharacteristicsDoc&) = delete;
    PerformanceCharacteristicsDoc(PerformanceCharacteristicsDoc&&) = delete;
    PerformanceCharacteristicsDoc& operator=(PerformanceCharacteristicsDoc&&) = delete;

    bool enabled_ = false;
    std::string config_;
};

} // namespace voltron::utility::documentation
