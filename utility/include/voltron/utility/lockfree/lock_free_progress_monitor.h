#pragma once

#include <string>
#include <vector>
#include <functional>
#include <source_location>

namespace voltron::utility::lockfree {

/**
 * @brief Ensure lock-free progress guarantees
 * 
 * Part of the Voltron C++ Utility Toolkit - comprehensive diagnostic infrastructure
 * for professional C++23 development.
 * 
 * @category Lockfree
 * @version 1.0.0
 */
class LockFreeProgressMonitor {
public:
    /**
     * @brief Get singleton instance
     */
    static LockFreeProgressMonitor& instance();

    /**
     * @brief Initialize the utility
     * @param config Optional configuration parameters
     */
    void initialize(const std::string& config = "");

    /**
     * @brief Shutdown the utility and cleanup resources
     */
    void shutdown();

    /**
     * @brief Check if the utility is currently enabled
     */
    bool isEnabled() const;

    /**
     * @brief Enable the utility
     */
    void enable();

    /**
     * @brief Disable the utility
     */
    void disable();

    /**
     * @brief Get utility statistics/status
     */
    std::string getStatus() const;

    /**
     * @brief Reset utility state
     */
    void reset();

private:
    LockFreeProgressMonitor() = default;
    ~LockFreeProgressMonitor() = default;
    
    // Non-copyable, non-movable
    LockFreeProgressMonitor(const LockFreeProgressMonitor&) = delete;
    LockFreeProgressMonitor& operator=(const LockFreeProgressMonitor&) = delete;
    LockFreeProgressMonitor(LockFreeProgressMonitor&&) = delete;
    LockFreeProgressMonitor& operator=(LockFreeProgressMonitor&&) = delete;

    bool enabled_ = false;
    std::string config_;
};

} // namespace voltron::utility::lockfree
