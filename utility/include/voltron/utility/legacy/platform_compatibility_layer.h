#pragma once

#include <string>
#include <vector>
#include <functional>
#include <source_location>

namespace voltron::utility::legacy {

/**
 * @brief Debug platform abstraction layers
 * 
 * Part of the Voltron C++ Utility Toolkit - comprehensive diagnostic infrastructure
 * for professional C++23 development.
 * 
 * @category Legacy
 * @version 1.0.0
 */
class PlatformCompatibilityLayer {
public:
    /**
     * @brief Get singleton instance
     */
    static PlatformCompatibilityLayer& instance();

    /**
     * @brief Initialize the utility
     * @param config Optional configuration parameters
     */
    void initialize(const std::string& config = "");

    /**
     * @brief Shutdown the utility and cleanup resources
     */
    void shutdown();

    /**
     * @brief Check if the utility is currently enabled
     */
    bool isEnabled() const;

    /**
     * @brief Enable the utility
     */
    void enable();

    /**
     * @brief Disable the utility
     */
    void disable();

    /**
     * @brief Get utility statistics/status
     */
    std::string getStatus() const;

    /**
     * @brief Reset utility state
     */
    void reset();

private:
    PlatformCompatibilityLayer() = default;
    ~PlatformCompatibilityLayer() = default;
    
    // Non-copyable, non-movable
    PlatformCompatibilityLayer(const PlatformCompatibilityLayer&) = delete;
    PlatformCompatibilityLayer& operator=(const PlatformCompatibilityLayer&) = delete;
    PlatformCompatibilityLayer(PlatformCompatibilityLayer&&) = delete;
    PlatformCompatibilityLayer& operator=(PlatformCompatibilityLayer&&) = delete;

    bool enabled_ = false;
    std::string config_;
};

} // namespace voltron::utility::legacy
