...
return value  # ⚠️ Warning: 'value' may be uninitialized
...
spawn
    set counter to counter plus 1  # ⚠️ Race detected!
end
...
=== Hotspots ===
⚠️ process_data:42 - Allocates in loop (consider pre-allocation)
⚠️ calculate:15 - Expensive repeated call (consider caching)
```
...
- ⚠️ Possible deadlock (requires design discipline)
...
# ⚠️ Compiler warning if free missing
free ptr
```
...
- ⚠️ Manual memory possible (with warnings)
- ⚠️ Raw pointers possible (discouraged)
...
**Guarantees:**
- ❌ No automatic safety
- ⚠️ Debug instrumentation available
- ✅ Full control
