# NLPL Documentation Organization Guide

## Folder Structure

```
docs/
├── 1_introduction/          # What is NLPL? Philosophy, overview, getting started
├── 2_language_basics/       # Core syntax, variables, scoping
├── 3_core_concepts/         # Advanced features (behaviors, events, game objects, etc.)
├── 4_architecture/          # Compiler design, backends, technical architecture
├── 5_type_system/          # Type system design, inference, generics
├── 6_module_system/        # Module system, imports, namespaces
├── 7_development/          # Developer guides, setup, style guide
├── 8_planning/             # Roadmaps, requirements, development plans
├── 9_status_reports/       # Progress reports, session results, milestones
├── 10_assessments/         # Analysis documents, comparisons, readiness assessments
└── Creating a Truly Natural Language Programming Language/  # Historical/archive
```

## Document Categories

### 1_introduction/ - Getting Started
- philosophy.md
- overview.md
- getting_started.md
- key_features.md

### 2_language_basics/ - Syntax & Fundamentals
- syntax_overview.md
- variables_and_objects.md
- commands_and_phrases.md
- scoping_and_blocks.md

### 3_core_concepts/ - Domain-Specific Features
- behaviors.md
- events_and_triggers.md
- game_objects.md
- database_operations.md
- file_operations.md
- networking.md
- time_and_frames.md

### 4_architecture/ - Technical Design
- compiler_architecture.md
- backend_strategy.md
- language_specification.md (formal spec)
- syntax_design.md

### 5_type_system/ - Type System
- type_system.md
- type_system_summary.md
- generic_type_system_completion.md

### 6_module_system/ - Modules & Organization
- module_system.md
- module_system_summary.md
- module_system_enhancements.md

### 7_development/ - Developer Resources
- DEVELOPMENT_SETUP.md
- style_guide.md
- FIXES_SUMMARY.md

### 8_planning/ - Project Planning
- requirements_analysis.md
- comprehensive_development_plan.md
- implementation_roadmap.md
- current_priorities.md

### 9_status_reports/ - Progress Tracking
- PROGRESS_REPORT.md
- COMPILER_MILESTONE.md
- SESSION_RESULTS.md
- project_reorganization_summary.md
- reorganization_status.md

### 10_assessments/ - Analysis & Comparisons
- WHITEPAPER_READINESS_ASSESSMENT.md
- error_handling_assessment.md
- existing_approaches.md
- nlpl_vs_engpp.md
- examples_and_comparisons.md
